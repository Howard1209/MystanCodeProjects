...
def search_names(name_data, target):
    ...
    matching_names = []
    ta = target.upper()
    for name in name_data:   # 一開始寫name_data[0] 有想錯再思考
        revise_name = name.upper()
        for i in range(len(revise_name)-len(ta)+1):
            if ta == revise_name[i:len(ta)+i]:
